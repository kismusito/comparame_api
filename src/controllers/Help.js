                ...
                if (ProductID) {
                    ...
                    if (checkSupermarker._id == checkProduct.supermarker) {
                        ...
                        if (checkNewCategory) {
                            ...
                            return res.status(400).json({
                                status: false,
                                message: "La Categoria a la que intenta cambiar no existe'",
                            });
                        }
                    } else {
                        return res.status(400).json({
...
ProductMethods.deleteProduct = async (req, res) => {
    try {
        const permissions = Permission.can(req.user.rol.name).deleteOwn("product").granted
        if (permissions) {
            const {
                ProductID
            } = req.body;
...
export {
    ProductMethods as ProductController
};